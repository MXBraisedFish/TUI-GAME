...
if [[ "$REG_CONFIRM" =~ ^[Yy]$ ]]; then
    ...
    SYMLINK_PATHS=("/usr/local/bin/tg" "$HOME/bin/tg" "$HOME/.local/bin/tg")
    ...
    # Check for Applications symlink (macOS style)
    APP_SYMLINK="$HOME/Applications/TUI-GAME"
    if [ -L "$APP_SYMLINK" ] && [ "$(readlink -f "$APP_SYMLINK")" = "$SCRIPT_DIR" ]; then
        rm -f "$APP_SYMLINK" && echo "[OK] Removed Application symlink" || echo "[ERROR] Failed to remove Application symlink"
    fi
...
if [ $REG_REMOVED -eq 0 ]; then
    echo "[REMINDER] Please manually remove any system integration (symlinks, etc.) if needed."
fi
