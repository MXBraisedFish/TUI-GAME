...
echo "[3/8] Unzipping files into: $INSTALL_DIR..."
...
read -p "Do you want to add this directory to your PATH (~/.zshrc) automatically? (Y/N): " choice
if [[ "$choice" =~ ^[Yy]$ ]]; then
    if ! grep -q "$INSTALL_DIR" ~/.zshrc 2>/dev/null; then
        echo "export PATH=\"\$PATH:$INSTALL_DIR\"" >> ~/.zshrc
        echo "[SUCCESS] Path registered to ~/.zshrc."
    else
        echo "[INFO] Path already exists in ~/.zshrc."
    fi
    ADD_TO_PATH="Y"
else
...
read -p "Do you want to remove the directory from ~/.zshrc? (Y/N): " reg_confirm
if [[ "\$reg_confirm" =~ ^[Yy]$ ]]; then
    # 使用 sed 删除配置文件中的路径，macOS 下 sed -i 必须跟空字符串
    sed -i '' "\|$INSTALL_DIR|d" ~/.zshrc
    echo "[SUCCESS] PATH cleaned."
else
    echo "[INFO] Please clean ~/.zshrc manually if needed."
fi
echo "Bye bye."
read -n 1 -s -r -p "Press any key to exit..."
rm -- "\$0"
