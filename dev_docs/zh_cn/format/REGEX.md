...
本文档说明 TUI GAME `string` 库中两类模式匹配 API 使用的语法规则：

- **Lua 模式**：用于 `string.find`、`string.match`、`string.gmatch`、`string.gsub`。
- **正则表达式**：用于 `string.regex_*` 系列 API。

当前正则表达式实现基于 Rust `regex 1.13.1`，使用 Unicode 字符串正则表达式。它不是 Lua Pattern，也不完全等同于 PCRE、JavaScript、Python 或其他回溯型正则引擎。
...
Lua 模式是 Lua 语言内置的轻量级匹配语法，用于 `string.find`、`string.match`、`string.gmatch` 和 `string.gsub`。它不同于正则表达式，语法更简洁，但功能也更受限。

## 1. 适用 API

| API | 作用 |
| --- | --- |
| `string.find` | 查找第一个匹配，返回起止位置和捕获表 |
| `string.match` | 返回第一个匹配的捕获内容；没有捕获组时返回完整匹配 |
| `string.gmatch` | 创建迭代器，依次返回所有非重叠匹配 |
| `string.gsub` | 替换所有或指定数量的匹配 |

## 2. Lua 字符串中的转义

Lua 模式使用 `%` 作为转义字符，而不是反斜杠。在普通 Lua 引号字符串中，`%` 不需要额外转义：
...
以上字符类的大写版本（如 `%A`、`%D`）表示对应类的补集。

### 示例

```lua
local found = string.find { text = "Hello 123", pattern = "%d+" }
-- found.captures[1] == "123"
```

### 额外补充

- Lua 模式**不**支持 Unicode 字符类（如 `\p{L}`）。`%a` 仅匹配 ASCII 字母。
- `.` 不匹配换行符，且按字节（UTF-8 编码单元）处理，而非 Unicode 码点。

## 4. 魔法字符与转义

以下字符在 Lua 模式中具有特殊含义，若需匹配字面量，必须在前面加上 `%` 进行转义：
...
用方括号括起的字符集合，匹配其中任意一个字符。

| 语法 | 含义 |
| --- | --- |
| `[abc]` | 匹配 `a`、`b` 或 `c` |
| `[^abc]` | 匹配除 `a`、`b`、`c` 外的任意字符 |
| `[a-z]` | 匹配 `a` 至 `z` 范围内任意字符 |
| `[0-9a-zA-Z_]` | 多个范围和字符的并集 |
| `[%^%]]` | 匹配 `^` 或 `]`（`]` 需转义） |
| `[%-%]]` | 匹配 `-` 或 `]`（`-` 在中间需转义） |
...
- 在字符集合中，`-` 若出现在开头或结尾，则作为普通字符匹配，无需转义。
- `]` 在集合中必须转义为 `%]`。
- `^` 若不在集合开头，则作为普通字符。

## 6. 重复修饰符

修饰符用于指定前一个字符类或分组的重复次数。
...
```lua
-- 贪婪匹配：尽量多匹配数字
local g1 = string.match { text = "12345abc", pattern = "(%d*)" }
-- g1[1] == "12345"

-- 非贪婪匹配：尽量少匹配数字
local g2 = string.match { text = "12345abc", pattern = "(%d-)" }
-- g2[1] == ""
```

### 额外补充

- `-` 是 Lua 模式独有的非贪婪修饰符，正则表达式中使用 `*?` 实现类似效果。
- `?` 仅匹配零次或一次，没有懒惰版本。

## 7. 捕获

用圆括号括起的子模式称为捕获组，匹配成功后捕获对应子串。

| 语法 | 含义 |
| --- | --- |
| `(pattern)` | 捕获匹配的内容，按左括号顺序编号 |
| `()` | 空捕获，匹配当前位置，返回该位置索引（整数） |
...
```lua
local m = string.match { text = "Name: Alice, Age: 30", pattern = "Name: (%w+), Age: (%d+)" }
-- m[1] == "Alice"
-- m[2] == "30"
-- m.n == 2
```
...
```lua
local b1 = string.test { text = "123", pattern = "^%d+$" }
-- b1 == true

local b2 = string.test { text = "abc123", pattern = "^%d+$" }
-- b2 == false
```
...
| 语法 | 含义 |
| --- | --- |
| `%bxy` | 匹配从 `x` 开始到 `y` 结束的平衡字符串，其中 `x` 和 `y` 是任意两个字符 |

### 示例

```lua
local balanced = string.match { text = "foo(bar(baz))qux", pattern = "%b()" }
-- balanced[1] == "(bar(baz))"
```

## 10. 替换引用

在 `string.gsub` 中，替换字符串 `repl` 支持以下特殊引用：
...
> **重要**：本框架的 `string.gsub` 使用 `$` 作为引用前缀，这与标准 Lua 的 `%0`/`%1` 不同。标准 Lua 行为在此处不适用。
...
本部分说明 `string.regex_*` 系列 API 使用的正则表达式语法。当前实现基于 Rust `regex 1.13.1`，使用 Unicode 字符串正则表达式。它不是 Lua Pattern，也不完全等同于 PCRE、JavaScript、Python 或其他回溯型正则引擎。
