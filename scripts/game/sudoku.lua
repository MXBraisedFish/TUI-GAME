...
local function ts()
  local w, h = 120, 40; if type(get_terminal_size) == "function" then
    local tw, th = get_terminal_size()
    if type(tw) == "number" and type(th) == "number" then w, h = tw, th end
  end; return w, h
end
local function key(k)
  if k == nil then return "" end; if type(k) == "string" then return string.lower(k) end; return tostring(k):lower()
end
...
local function sh(t) for i = #t, 2, -1 do
    local j = rnd(i) + 1; t[i], t[j] = t[j], t[i]
  end end
local function sec()
  local e = S.ef or S.f; return math.max(0, math.floor((e - S.sf) / FPS))
end
...
local function okv(b, r, c, n)
  ...
  local br = math.floor((r - 1) / B) * B + 1; local bc = math.floor((c - 1) / B) * B + 1
  for i = br, br + B - 1 do for j = bc, bc + B - 1 do if (i ~= r or j ~= c) and b[i][j] == n then return false end end end
  return true
end
local function cand(b, r, c)
  local t = {}
  for n = 1, 9 do if okv(b, r, c, n) then t[#t + 1] = n end end
  return t
end
local function pick(b)
  local br, bc, bt = nil, nil, nil; for r = 1, N do for c = 1, N do if b[r][c] == 0 then
        local t = cand(b, r, c)
        if #t == 0 then return r, c, t end; if bt == nil or #t < #bt then
          br, bc, bt = r, c, t; if #bt == 1 then return br, bc, bt end
        end
      end end end; return br, bc, bt
end
local function fill(b)
  local r, c, t = pick(b)
  if r == nil then return true end; if t == nil or #t == 0 then return false end; sh(t)
  for i = 1, #t do
    b[r][c] = t[i]
    if fill(b) then return true end
  end; b[r][c] = 0; return false
end
local function csol(b, lim)
  local cnt = 0; local function dfs()
    if cnt >= lim then return end; local r, c, t = pick(b)
    if r == nil then
      cnt = cnt + 1; return
    end; if t == nil or #t == 0 then return end; for i = 1, #t do
      b[r][c] = t[i]; dfs(); if cnt >= lim then
        b[r][c] = 0; return
      end
    end; b[r][c] = 0
  end; dfs(); return cnt
...
local function gen_solved()
  local b = mx(0); fill(b); return b
end
local function dig_unique(sol, h)
  local p = cp(sol)
  local co = {}
  for r = 1, N do for c = 1, N do co[#co + 1] = { r = r, c = c } end end; sh(co)
  local rm = 0; for i = 1, #co do
    if rm >= h then break end; local r, c = co[i].r, co[i].c; local o = p[r][c]; p[r][c] = 0; local t = cp(p); if csol(t, 2) == 1 then rm =
      rm + 1 else p[r][c] = o end
  end
  return p, rm
end
local function dig_any(sol, h)
  ...
  local co = {}
  for r = 1, N do for c = 1, N do co[#co + 1] = { r = r, c = c } end end; sh(co)
  for i = 1, math.min(h, #co) do p[co[i].r][co[i].c] = 0 end; return p
end
local function gen(d)
  local h = HOLES[d] or 50; local fs = nil; for _ = 1, 6 do
    local s = gen_solved(); fs = s; local p, rm = dig_unique(s, h); if rm >= h then return p, s end
  end; local s = fs or gen_solved(); return dig_any(s, h), s
end
local function recf()
  S.cf = mx(false)
  local function mark(ls) if #ls > 1 then for i = 1, #ls do S.cf[ls[i].r][ls[i].c] = true end end end
  for r = 1, N do
    local mp = {}
    for c = 1, N do
      local v = S.b[r][c]
      if v > 0 then
        mp[v] = mp[v] or {}; mp[v][#mp[v] + 1] = { r = r, c = c }
      end
    end
  ...
  for br = 1, N, 3 do for bc = 1, N, 3 do
      local mp = {}
      for r = br, br + 2 do for c = bc, bc + 2 do
          local v = S.b[r][c]
          if v > 0 then
            mp[v] = mp[v] or {}; mp[v][#mp[v] + 1] = { r = r, c = c }
          end
...
local function done()
  for r = 1, N do for c = 1, N do if S.b[r][c] == 0 or S.cf[r][c] then return false end end end
  return true
end
local function rep(old, new)
  if old == nil then return true end; if new.d ~= old.d then return new.d > old.d end; return new.t < old.t
end
local function load_best()
  if type(load_data) ~= "function" then return nil end; local ok, d = pcall(load_data, "sudoku_best")
  if (not ok) or type(d) ~= "table" then return nil end; local lv = math.floor(tonumber(d.d) or tonumber(d.difficulty) or
  0)
  local tm = math.floor(tonumber(d.t) or tonumber(d.min_time_sec) or 0)
  if lv < 1 or lv > 5 or tm <= 0 then return nil end; return { d = lv, t = tm }
end
local function save_best(x) if type(save_data) == "function" then pcall(save_data, "sudoku_best",
      { d = x.d, t = x.t, difficulty = x.d, min_time_sec = x.t }) end end
local function cbest()
  if S.bc or not S.win then return end; local n = { d = S.d, t = sec() }
  if rep(S.best, n) then
    S.best = n; save_best(n)
  end; if type(update_game_stats) == "function" then
    local sc = S.d * 100000 - n.t; if sc < 0 then sc = 0 end; pcall(update_game_stats, "sudoku", sc, n.t)
  end; S.bc = true
end
local function chk()
  local w = done(); if w then if not S.win then
      S.win = true; S.ef = S.f; S.bc = false; cbest()
    end else
    S.win = false; S.ef = nil; S.bc = false
  end
end
local function pushu(r, c, o, n)
  if o == n then return end; S.undo[#S.undo + 1] = { r = r, c = c, o = o, n = n }; while #S.undo > UL do table.remove(
    S.undo, 1) end
end
local function snap() return { d = S.d, p = cp(S.p), s = cp(S.sol), b = cp(S.b), g = cp(S.g), r = S.r, c = S.c, e = sec(), w =
  S.win, u = S.undo, a = S.as, hl = S.hl } end
local function vm(m)
  if type(m) ~= "table" then return false end; for r = 1, N do
    if type(m[r]) ~= "table" then return false end; for c = 1, N do
...
local function restore(x)
  if type(x) ~= "table" then return false end; local d = math.floor(tonumber(x.d) or tonumber(x.difficulty) or 0)
  if d < 1 or d > 5 then return false end
  local p = x.p or x.puzzle; local b = x.b or x.board; if (not vm(p)) or (not vm(b)) then return false end
  S.d = d; S.p = cp(p); S.b = cp(b); local s = x.s or x.solution; if vm(s) then S.sol = cp(s) else S.sol = cp(S.p) end
  local g = x.g or x.given; S.g = mx(false)
  if vm(g) then for r = 1, N do for c = 1, N do S.g[r][c] = g[r][c] and true or false end end else for r = 1, N do for c = 1, N do S.g[r][c] =
        S.p[r][c] ~= 0 end end end
  S.r = math.max(1, math.min(9, math.floor(tonumber(x.r) or 1))); S.c = math.max(1,
    math.min(9, math.floor(tonumber(x.c) or 1)))
  S.hl = (x.hl == true)
  S.undo = {}; local u = x.u or x.undo_stack; if type(u) == "table" then for i = 1, #u do
      local e = u[i]
      if type(e) == "table" then
        ...
        if r >= 1 and r <= 9 and c >= 1 and c <= 9 and o >= 0 and o <= 9 and n >= 0 and n <= 9 then S.undo[#S.undo + 1] = { r =
          r, c = c, o = o, n = n } end
      end
    end end
...
local function save_state(show)
  local ok = false; local x = snap();
  if type(save_game_slot) == "function" then
    local s, r = pcall(save_game_slot, "sudoku", x); ok = s and r ~= false
  elseif type(save_data) == "function" then
    local s, r = pcall(save_data, "sudoku", x); ok = s and r ~= false
  end
  if show then
...
local function load_state()
  ...
  if type(load_game_slot) == "function" then
    local s, r = pcall(load_game_slot, "sudoku"); ok = s and r ~= nil; x = r
  elseif type(load_data) == "function" then
    local s, r = pcall(load_data, "sudoku"); ok = s and r ~= nil; x = r
  end
  if ok then return restore(x) end; return false
end
local function lmode()
  if type(get_launch_mode) ~= "function" then return "new" end; local ok, m = pcall(get_launch_mode)
  if (not ok) or type(m) ~= "string" then return "new" end; m = string.lower(m)
  if m == "continue" then return "continue" end; return "new"
end
...
local function setv(r, c, n)
  ...
  if o == n then return false end; pushu(r, c, o, n); S.b[r][c] = n; recf(); chk(); S.dirty = true; return true
end
local function undo()
...
local function wrap(t, w)
  if w <= 1 then return { t } end; local ls, cur, had = {}, "", false; for tok in string.gmatch(t, "%S+") do
    had = true; if cur == "" then cur = tok else
      local c = cur .. " " .. tok; if wid(c) <= w then cur = c else
        ls[#ls + 1] = cur; cur = tok
      end
    end
  end; if not had then return { "" } end; if cur ~= "" then ls[#ls + 1] = cur end; return ls
end
local function minw(t, m, h)
  local f = wid(t)
  local w = h
  while w <= f do
    if #wrap(t, w) <= m then return w end; w = w + 1
  end; return f
end
local function fr(x, y, w, h, bg)
  if w <= 0 or h <= 0 then return end; local ln = string.rep(" ", w)
...
local function geo()
  ...
  return { tw = tw, th = th, bx = bx, by = sy + 3, bw = bw, bh = bh, best = best, st = st, tx = tx, ty = sy, ny = sy + 3 +
  bh, cl = cl, thh = thh, rw = math.max(bw, topw, nw, minw(ctl, 3, 40)) + 2, rh = thh + 1 }
end
local function drow(bx, y, r)
  local p = tostring(r) .. " - "; draw_text(bx, y, p .. "║", "white", "black"); local x = bx + wid(p .. "║")
  local crb = math.floor((S.r - 1) / B); local ccb = math.floor((S.c - 1) / B); local rb = math.floor((r - 1) / B)
  for c = 1, N do
    local v = S.b[r][c]; local t = (v > 0) and (" " .. tostring(v)) or "  "; local fg = "white"; local cur = (r == S.r and c == S.c)
    if v > 0 then
      if cur then
    ...
    end; local bg = "black"; if cur then
      bg = "light_yellow"
    elseif S.hl then
      local cb = math.floor((c - 1) / B)
      if r == S.r or c == S.c or (rb == crb and cb == ccb) then bg = "#B3B3B3" end
    end; draw_text(x, y, t, fg, bg); x = x + 2; if c == 3 or c == 6 then
      draw_text(x, y, "│", "white", "black"); x = x + 1
    end
  end
  draw_text(x, y, "║", "white", "black")
end
local function dboard(g)
  local x, y = g.bx, g.by; draw_text(x, y, H1, "white", "black"); draw_text(x, y + 1, H2, "white", "black"); draw_text(
  x, y + 2, BT, "white", "black"); local ry = y + 3; for r = 1, N do
    drow(x, ry, r); ry = ry + 1; if r == 3 or r == 6 then
      draw_text(x, ry, BM, "white", "black"); ry = ry + 1
    end
  end; draw_text(x, ry, BB, "white", "black")
end
local function dnotice(g)
  ...
  if l ~= "" then
    local x = math.floor((g.tw - wid(l)) / 2); if x < 1 then x = 1 end; draw_text(x, g.ny, l, col, "black")
  end
end
local function dcontrols(g)
  local by = g.ny + 1; for i = 0, 2 do draw_text(1, by + i, string.rep(" ", g.tw), "white", "black") end
  local off = 0; if #g.cl < 3 then off = math.floor((3 - #g.cl) / 2) end
  for i = 1, #g.cl do
    local ln = g.cl[i]; local x = math.floor((g.tw - wid(ln)) / 2); if x < 1 then x = 1 end; draw_text(x, by + off + i -
    1, ln, "white", "black")
  end
end
local function clear_area() if S.area then fr(S.area.x, S.area.y, S.area.w, S.area.h, "black") end end
local function render()
  local g = geo(); local a = { x = 1, y = g.ty, w = g.tw, h = g.thh }
  if S.area == nil then fr(a.x, a.y, a.w, a.h, "black") elseif S.area.w ~= a.w or S.area.h ~= a.h or S.area.y ~= a.y then
    clear_area(); fr(a.x, a.y, a.w, a.h, "black")
  end
  S.area = a
  draw_text(1, g.ty, string.rep(" ", g.tw), "white", "black")
  draw_text(1, g.ty + 1, string.rep(" ", g.tw), "white", "black")
  local bx = math.floor((g.tw - wid(g.best)) / 2); if bx < 1 then bx = 1 end; draw_text(bx, g.ty, g.best, "dark_gray",
    "black")
...
local function size_ok()
  local g = geo(); local tw, th, mw, mh = g.tw, g.th, g.rw, g.rh
  if tw >= mw and th >= mh then return true end
  clear(); warn(tw, th, mw, mh); return false
end
local function sync_resize()
  local w, h = ts()
  if w ~= S.tw or h ~= S.th then
    S.tw, S.th = w, h; clear(); S.area = nil; S.dirty = true
  end
end
local function refresh()
  local e = sec(); if e ~= S.le then
    S.le = e; S.dirty = true
  end
  local tv = (S.toast ~= nil and S.f <= S.tu); if tv ~= S.lt then
    S.lt = tv; S.dirty = true
  end
end
local function autosave()
  if S.win then return end; local e = sec(); if e - S.as >= 60 then
    save_state(false); S.as = e
  end
end
local function hmode(k)
  if k == "esc" or k == "q" then
    S.im = nil; S.ib = ""; S.dirty = true; return "changed"
  end
  if k == "backspace" or k == "delete" then
    if #S.ib > 0 then
      S.ib = string.sub(S.ib, 1, #S.ib - 1); S.dirty = true
    end; return "changed"
  end
  if k == "enter" then
    if S.im == "difficulty" then
      local d = math.floor(tonumber(S.ib) or 0); S.im = nil; S.ib = ""; if d >= 1 and d <= 5 then reset(d) else S.dirty = true end; return
      "changed"
    end
    if S.im == "jump" then
      local a, b = S.ib:match("^(%d+)%s+(%d+)$"); S.im = nil; S.ib = ""; if a and b then
        S.r = math.max(1, math.min(9, math.floor(tonumber(a) or 1))); S.c = math.max(1,
          math.min(9, math.floor(tonumber(b) or 1)))
      end; S.dirty = true; return "changed"
    end
  end
  if S.im == "difficulty" then
  ...
  if S.im == "jump" then
    if k:match("^%d$") or k == "space" then
      local t = (k == "space") and " " or k; if #S.ib < 8 then
        S.ib = S.ib .. t; S.dirty = true
      end
    end; return "changed"
  end
  return "none"
...
local function input(k)
  ...
  if k == "p" then
    S.im = "difficulty"; S.ib = ""; S.dirty = true; return "changed"
  end
  if k == "d" then
    S.im = "jump"; S.ib = ""; S.dirty = true; return "changed"
  end
  if k == "a" then
    undo(); return "changed"
  end
  ...
  if k == "down" then
    S.r = math.min(9, S.r + 1); S.dirty = true; return "changed"
  end
  if k == "left" then
    S.c = math.max(1, S.c - 1); S.dirty = true; return "changed"
  end
  if k == "right" then
    S.c = math.min(9, S.c + 1); S.dirty = true; return "changed"
  end
  if k:match("^[1-9]$") then
    setv(S.r, S.c, tonumber(k)); return "changed"
  end
  if k == "space" then
    setv(S.r, S.c, 0); return "changed"
  end
  return "none"
end
local function init()
  clear(); S.tw, S.th = ts(); S.best = load_best(); S.launch = lmode()
  if S.launch == "continue" then if not load_state() then reset(3) end else reset(3) end
  S.dirty = true
end
local function loop()
  while true do
    ...
    if size_ok() then
      local a = input(k); if a == "exit" then return end
      sync_resize(); autosave(); refresh(); if S.dirty then
